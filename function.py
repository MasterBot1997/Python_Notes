...
def filter_date(filter, key, name_f):
    key = check_key(filter, key, name_f)
    with open("all_notes/" + name_f, "r", encoding="utf8") as f:
        reader = csv.DictReader(f)
        for row in reader:
            if row[filter] == key:
...
def specific_filter_date(filter, key, name_f, n):
    key = check_key_s(filter, key, name_f, n)
    with open("all_notes/" + name_f, "r", encoding="utf8") as f:
        reader = csv.DictReader(f)
        for row in reader:
            if n == "1":
                temp = row[filter].split(" ")[0]
                if temp == key:
                    print(
                        row["id"],
                        row["header"],
                ...
                # Сохраняем значение год и месяц
                temp = (
                    row[filter].split(" ")[0].split("-")[0]
                    + "-"
                    + row[filter].split(" ")[0].split("-")[1]
                )
                if temp == key:
                    print(
                        row["id"],
                ...
                # Сохраняем значение года
                temp = row[filter].split(" ")[0].split("-")[0]
                if temp == key:
                    print(
                        row["id"],
                        row["header"],
                ...
                # Сохраняем значение года
                temp = row[filter].split(" ")[0].split("-")[0]
                if int(temp) >= int(key.split("-")[0]) <= int(key.split("-")[1]):
                    print(
                        row["id"],
                        row["header"],
                        row["the_note"],
