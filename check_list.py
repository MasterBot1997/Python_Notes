...
def check_key(filter, num, name_f):
    while True:
        with open("all_notes/" + name_f, "r", encoding="utf8") as f:
            ...
            for row in reader:
                if row[filter] == num:
                    return num
                elif row[filter].split(' ')[0] == num:
                    return num
                elif row[filter].split(' ')[0].split('-')[0] +'-'+ row[filter].split(' ')[0].split('-')[1] == num:
                    return num
                elif row[filter].split(' ')[0].split('-')[0] == num:
                    return num
            else:
...
def check_key_s(filter, num, name_f, n):
    while True:
        with open("all_notes/" + name_f, "r", encoding="utf8") as f:
            reader = csv.DictReader(f)
            for row in reader:
                if row[filter] == num and n == '1':
                    return num
                elif row[filter].split(' ')[0] == num:
                    return num
                elif row[filter].split(' ')[0].split('-')[0] +'-'+ row[filter].split(' ')[0].split('-')[1] == num and n == '2':
                    return num
                elif row[filter].split(' ')[0].split('-')[0] == num and n == '3':
                    return num
                elif  num.split('-')[0].isdigit() and num.split('-')[1].isdigit():
                    if int(row[filter].split(' ')[0].split('-')[0]) >= int(num.split('-')[0]) <= int(num.split('-')[1]) and n == '4':
                        return num
            else:
                logging.warning("Введен несуществующий параметр")
                print("\n'Такого параметра не существует!'")
